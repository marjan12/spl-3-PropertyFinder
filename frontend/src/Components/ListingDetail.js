    ...
    async function GetListingInfo() {
      try {
        const response = await Axios.get(
          `http://127.0.0.1:8001/api/listings/${params.id}/`
        );
  ...
  useEffect(() => {
    if (state.listingInfo) {
      async function GetProfileInfo() {
        try {
          const response = await Axios.get(
            `http://127.0.0.1:8001/api/profiles/${state.listingInfo.seller}/`
          );
    ...
    if (confirmDelete) {
      try {
        const response = await Axios.delete(
          `http://127.0.0.1:8001/api/listings/${params.id}/delete/`
        );
    ...
    >
      ...
      {state.listingInfo.description ? (
        ...
        >
          ...
          <Typography variant="" style={{ marginTop: "2rem" }}>
            {state.listingInfo.description}
          </Typography>
        </Grid>
      ) : (
